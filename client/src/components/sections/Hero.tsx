  ...
  return (
    <section className="relative min-h-[80vh] flex items-center pt-20 pb-16 overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10 grid lg:grid-cols-12 gap-12 items-center">
        ...
        >
          <p className="text-sm tracking-[0.2em] uppercase text-amber-200 font-semibold mb-4">
            Inbridge Nigeria Ltd
          </p>
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold leading-tight mb-6">
            EPC, Energy Solutions & <span className="text-amber-300">Mineral Exports</span>
          </h1>
          ...
          <div className="flex flex-wrap gap-3 mb-8">
            {["EPC & Project Delivery", "Gas & Power Solutions", "Ilmenite & Mineral Exports"].map((tag) => (
              <span key={tag} className="px-4 py-1.5 rounded-full border border-amber-300/30 bg-amber-100/10 backdrop-blur-sm text-sm text-amber-100">
                {tag}
              </span>
            ))}
          </div>
          ...
          <div className="flex flex-wrap gap-4 mb-10">
            <Button size="lg" className="rounded-full text-base px-8 bg-amber-500 hover:bg-amber-600 shadow-lg shadow-amber-900/40 text-white">
              <a href="#contact-cta">Request a Quote</a>
            </Button>
            <Button size="lg" variant="outline" className="rounded-full text-base px-8 border-amber-300 text-white hover:bg-amber-100/10 bg-transparent">
              <a href="#what-we-do">Explore Our Services</a>
            </Button>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 text-sm text-amber-100 border-t border-amber-200/20 pt-8">
            <div>
        ...
        >
          <Card className="bg-amber-900/90 border-amber-700/50 backdrop-blur-xl shadow-2xl shadow-black/50 text-white">
            <CardContent className="p-8">
              ...
              <ul className="space-y-3 mb-6 text-sm text-amber-100">
                {[
              ...
              <div className="bg-amber-800/50 rounded-lg p-4 border border-amber-700/50">
